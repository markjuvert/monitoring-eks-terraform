...
![monitoring](images/monitor2.png)

## Overview of the Project
In this project, AWS Cloudwatch Agent is used to monitor metrics of the cluster and FluentBit is used to monitor the application logs. In my previous demonstrations, I created the infrastructure and deployed a sample app. For more information about creation of the infrastructure, please click the link.
In the EKS cluster, the cloudwatch agent will be deployed in a namespcace using configmap and service account. The Cluster role will be added to the service account using cluster role binding to give the necessary permissions to collect daemonsets related metrics of pods running in the cluster to send metrics to cloudwatch and cloudwatch logs.
In addition to that, the application logs can also be monitored using FluentBit. The FluentBit agent can also be deployed via configmaps and service account just like in CWA above.
In this project, the CWA and FB are deployed using kubectl and later deployed using Terraform to fully understand the concept of monitoring.


## INTRODUCTION TO AWS CLOUDWATCH INSIGHTS

AWS Cloud Watch Container Insights collects metrics and logs from containerized applications and micro-services deployed on AWS ECS, EKS Clusters and Kubernetes platforms on Amazon EC2.
AWS Cloud Watch Container Insights collects metrics such as CPU, Memory, Network, Node Disk Usage, Cluster Failures, Number Of Nodes, Number Of Pods, Number of Containers, EC2 Worker Nodes, and Fargate Pods.
...
For more details about the project, please checkout the [Project Link](http://www.monitor-cwa-fb.cloud2day.link/)
